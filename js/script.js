...
var updateSiteValue = document.getElementById("updateSiteValue");
var siteList = []
    ...
    for (let i = 0; i < siteList.length; i++) {
        cartoona += `<tr>
                            ...
                            <td>
                             ...
                             <td>
                            <button class="btn btn-primary px-3" onClick="loadSite(${i})">
                            <i class="fa-regular fa-pen-to-square me-md-2"></i>
                            UpDate</button>
                        </td>
                            <td>
...
function loadSite(updateIndex) {
    localUpdate = updateIndex
    siteName.value = siteList[updateIndex].name
    siteUrl.value = siteList[updateIndex].url
    updateSiteValue.classList.remove("d-none")
    submit.classList.add("d-none")
    validation(siteName)
    validation(siteUrl)
}
function updateSite() {
    siteList[localUpdate].name = siteName.value
    siteList[localUpdate].url = siteUrl.value
    updateSiteValue.classList.add("d-none")
...
function validation(elemnt) {
    ...
    if (validation[elemnt.id].test(elemnt.value)) {
        ...
        if (siteName.classList.contains("is-valid") && siteUrl.classList.contains("is-valid")) {
            ...
            updateSiteValue.disabled = false
        }
    }
    else {
